...
for(run in 1:num_rv){
  pitch.k0[run,1] <- runif(1,0.1,0.6)
  pitch.k0[run,2] <- runif(1,0.7,0.99)
}
...
mod_exp1_list[[1]]
mod_exp1_list[[1]]$mod$code
# best model out of the 60 I ran is the same as model mFL_2st
...
exp1_ps$yawRes[which(exp1_ps$yawRes==-Inf | is.na(exp1_ps$yawRes))]
yawRes_excl <- which(exp1_ps$yawRes==-Inf | is.na(exp1_ps$yawRes))
length(yawRes_excl)
yawRes_corr <- exp1_ps$yawRes[-yawRes_excl]
...
# Overall, the residuals are symmetrical and there is only a bit of 
# residual autocorrelation. Eleven of the pseudoresiduals for yaw are 
# NA but there doesn't seem to be a systematic problem in the distribution. 
...
# residual autocorrelation in step length. Nine of the pseudoresiduals for 
...
# bit worse behaved than in experiment 1.
